...
class VentanaSumaMatrices(Toplevel):
    def __init__(self, parent):
        super().__init__(parent)
        self.parent = parent
        self.title("Suma Matrices")
        ...
        # Crear frames para cada matriz y el signo de suma
        ...
        self.sum_label = ttk.Label(self.frame_sum_sign, text="+", font=("Arial", 24))
        self.sum_label.pack()
        ...
        self.calc_button = ttk.Button(self, text="Calcular Suma", command=self.process_matrix)
    ...
    def ask_dimensions(self):
        ...
        if rows and columns:  # Permitimos matrices no cuadradas
    ...
    def process_matrix(self):
        try:
            ...
            m1 = [[float(entry.get()) for entry in self.matriz1_entries[i]] for i in range(len(self.matriz1_entries))]
            m2 = [[float(entry.get()) for entry in self.matriz2_entries[i]] for i in range(len(self.matriz2_entries))]

            # Calcular la suma de las matrices y preparar los detalles del cálculo
            result = []
            calculation_details = []
            ...
            for i in range(len(m1)):
                ...
                for j in range(len(m1[0])):
                    sum_result = m1[i][j] + m2[i][j]
                    result_row.append(sum_result)
                    detail_row.append(f"{m1[i][j]} + {m2[i][j]} = {sum_result:.2f}")
            ...
            result_window.title("Procedimiento Suma")
